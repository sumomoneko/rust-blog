...
この誤解はライフタイムに関するというよりジェネリクスについての話ですが、ジェネリクスとライフタイムとはRustでは密接に絡み合っているので、一方について言及せず他方について言及することは不可能なのです。
...
ええ、しかしライフタイム`'static`_付きの_ 型と、ライフタイム`'static`で _境界付けられた_ 型とは異なるのです。後者は実行時に動的に確保され、安全に、かつ自由に変化させられ、ドロップでき、意図した通りの期間で有効なのです。

つまり`&'static T`と`T: 'static`を区別することが大切です。

`&'static T`はプログラムの最後までを含んだ限りなく長い間安全に保持される`T`への、不可変な参照です。これは`T`自身が不可変で _参照が作られた後に_ moveしない場合にのみ可能です。`T`はコンパイル時に作られる必要はないのです。メモリがリークするコストの元で、実行時にランダムに動的に確保されたデータを作り、それへの`'static`な参照を返すことが可能です。例えば
...
`T: 'static`はプログラム終了までを含んだ限りなく長い間で安全に保持される`T`です。`T: 'static`は`&'static T`を含みますが、同時に`String`や`Vec`などの所有型も含むのです。データの所有者は、データを保持している限りデータが無効になることがないことが保証されているので、プログラム終了までを含めて無期限にデータを保持することができ、安全にデータを保持することができます。`T: 'static`は、_"`T`はライフタイム`'static`を持っている"_ のではなく、 _"`T`はライフタイム`'static`によって境界付けられている"_ と読むべきなのです。以下にこうした概念を説明するためのプログラムを示します。
...
- `T: 'static`ならば`T`はライフタイム`'static`を持つ借用型 _または_ 所有型でありうる。
- `T: 'static`は所有型を含むので、`T`は
  - 実行時に動的に確保されてもよい
  - プログラム全体で有効である必要はない
  - 安全かつ自由に変更可能
  - 実行時に動的にドロップ出来る
  - (`'static`ではない)異なる期間のライフタイムを持っても良い

### 3) `&'a T`と`T: 'a`は同じ

この誤解は一つ前のものの一般化されたものです。

`&'a T`は暗黙的に`T: 'a`を要求します２。なぜならば、`T`自身がライフタイム`'a`を持たない場合、`T`へのライフタイム`'a`を持つ参照は有効にはならないからです。例えばRustコンパイラは`&'static Ref<'a, T>`型を許さないでしょう。なぜならば、`Ref`はライフタイム`'a`の期間しか有効でないため、`'a`より長い`'static`な参照を作ることができないからです。

つまり、`T: 'a`は`&'a T`を含みますがその逆は成り立ちません。
...
それぞれのバイトを借用でなくコピーすれば解決しそうです。ただ、今は`T`が単なるバイトなのでコピーで問題ないですが、コピーやクローンが高コストであったり不可能であったりする`T`に対しても、`ByteIter`を汎用的なスライスのイテレータとして使いたくなる時が来るかもしれません。しかしまぁ、これについてできることは無さそうです。このコードはコンパイル出来ているのでライフタイムの記述は正しいのでしょう。ね？
...
トレイトを実装した具体的な型は、参照を持つことが出来ます。参照を持つと言うことはライフタイム境界もあり、ゆえに対応するトレイトオブジェクトにもライフライム境界があります。
また、ライフタイムを明示した参照に対して直接トレイトを実装することもできます。
...
impl<'a, T> Trait for Ref<'a, T> {} // 参照を持つ型へのimpl Trait
```

何にせよ、初心者が関数をトレイトオブジェクトを使った関数からジェネリクスを使ったものへリファクタリングする際、またはその逆を行う際にしばしば混乱するため、この点については説明する価値があります。例として以下のプログラムを見てみましょう。
...
共有借用を引数にとる関数に対して、可変借用を渡すことが出来ます。Rustは暗黙的に、可変借用を共有借用として借用し直してくれるためです。
